...
function buildSummary(rows, mastery, wrongByTopic = []) {
  ...
  const weakTopics = (wrongByTopic || [])
    ...
    .map(x => ({
      ...
      grade: Number(x.grade || 1),
      subject: x.subject || 'math',
      semester: x.semester || 'upper',
...
function reportActions(db, user, weakTopics = []) {
  const actions = [];
  if (weakTopics[0]) actions.push({ type: 'weak_review', label: `生成「${weakTopics[0].topic}」悬赏`, topic: weakTopics[0].topic });
  actions.push({ type: 'praise', label: '发一张表扬卡' });
  actions.push({ type: 'boss_question', label: '出一道家长 Boss 题' });
  actions.push({ type: 'continue_campaign', label: '继续下一关' });
  const bounties = fullProduct.generateBounties(db, { user, source: 'report' }).slice(0, 3).map(r => ({ id: r.id, topic: r.topic, difficulty: r.difficulty, status: r.status }));
  return { actions, bounties };
}
...
function dailyRows(db, user, date) {
  return db.prepare(`SELECT lr.*, cl.topic, cl.title, ${DAY_EXPR} AS day
                    FROM level_runs lr
...
function wrongTopicContextSelect() {
  return `COALESCE(MAX(cl.grade), (
            SELECT wq.grade FROM wrong_questions wq
            WHERE wq.user=lra.user AND wq.topic=lra.topic AND wq.grade IS NOT NULL
            ORDER BY wq.created_at DESC LIMIT 1
          ), 1) AS grade,
          COALESCE(MAX(cl.subject), (
            SELECT wq.subject FROM wrong_questions wq
            WHERE wq.user=lra.user AND wq.topic=lra.topic AND COALESCE(wq.subject,'') <> ''
            ORDER BY wq.created_at DESC LIMIT 1
          ), 'math') AS subject,
          COALESCE(MAX(cl.semester), (
            SELECT wq.semester FROM wrong_questions wq
            WHERE wq.user=lra.user AND wq.topic=lra.topic AND COALESCE(wq.semester,'') <> ''
            ORDER BY wq.created_at DESC LIMIT 1
          ), 'upper') AS semester`;
}

function dailyWrongByTopic(db, user, date) {
  return db.prepare(`SELECT lra.topic, COUNT(*) AS wrong_count, ${wrongTopicContextSelect()}
                    FROM level_run_answers lra
                    LEFT JOIN level_runs lr ON lr.run_id=lra.run_id AND lr.user=lra.user
                    LEFT JOIN campaign_levels cl ON cl.id=lr.level_id
...
function dailyReportHandler(db, query = {}) {
  ...
  let date = query.date || dayString(new Date());
  let rows = dailyRows(db, user, date);
  // Backward compatibility for tests / old callers that pass UTC today's yyyy-mm-dd around Beijing midnight.
  const utcToday = new Date().toISOString().slice(0, 10);
  const bjToday = dayString(new Date());
  if (rows.length === 0 && query.date === utcToday && utcToday !== bjToday) {
    const fallback = dailyRows(db, user, bjToday);
    if (fallback.length) { rows = fallback; date = bjToday; }
  }
  if (rows.length === 0 && query.date === bjToday && utcToday !== bjToday) {
    const fallback = dailyRows(db, user, utcToday);
    if (fallback.length) { rows = fallback; date = utcToday; }
  }
  const wrongByTopic = dailyWrongByTopic(db, user, date);
  const mastery = fetchMastery(db, user, 5);
  const summary = buildSummary(rows, mastery, wrongByTopic);
  const feedback = reportActions(db, user, summary.weakTopics || []);
  return { status: 200, body: {
    date,
    period: { type: 'daily', startDate: date, endDate: date, days: 1 },
    summary,
    feedbackActions: feedback.actions,
    recommendedBounties: feedback.bounties,
...
function weeklyReportHandler(db, query = {}) {
  ...
  let rows = db.prepare(`SELECT lr.*, cl.topic, cl.title, ${DAY_EXPR} AS day
                           FROM level_runs lr
                           LEFT JOIN campaign_levels cl ON cl.id=lr.level_id
                           WHERE lr.user=?
  ...
  if (rows.length === 0 && endDate === utcToday && utcToday !== bjToday) {
    const fallback = db.prepare(`SELECT lr.*, cl.topic, cl.title, ${DAY_EXPR} AS day
                           FROM level_runs lr
                           LEFT JOIN campaign_levels cl ON cl.id=lr.level_id
                           WHERE lr.user=?
                             AND ${DAY_EXPR} BETWEEN date(?, '-6 day') AND date(?)
                             AND lr.result <> 'started'
                           ORDER BY COALESCE(lr.finished_at, lr.created_at) DESC`).all(user, bjToday, bjToday);
    if (fallback.length) { rows = fallback; effectiveEndDate = bjToday; }
  }
  const wrongByTopic = db.prepare(`SELECT lra.topic, COUNT(*) AS wrong_count, ${wrongTopicContextSelect()}
                                    FROM level_run_answers lra
                                    LEFT JOIN level_runs lr ON lr.run_id=lra.run_id AND lr.user=lra.user
                                    LEFT JOIN campaign_levels cl ON cl.id=lr.level_id
                                    WHERE lra.user=? AND lra.is_correct=0
                                    ...
                                    GROUP BY lra.topic
                                    ORDER BY wrong_count DESC`).all(user, effectiveEndDate, effectiveEndDate);
  const startDate = db.prepare(`SELECT date(?, '-6 day') AS d`).get(effectiveEndDate).d;
  const mastery = fetchMastery(db, user, 8);
  const summary = buildSummary(rows, mastery, wrongByTopic);
  const feedback = reportActions(db, user, summary.weakTopics || []);
  return { status: 200, body: {
    period: { type: 'weekly', startDate, endDate: effectiveEndDate, days: 7 },
    summary,
    feedbackActions: feedback.actions,
    recommendedBounties: feedback.bounties,
    runs: rows.map(r => ({ levelId: r.level_id, title: r.title, topic: r.topic, result: r.result, stars: r.stars, correct: r.correct_count, wrong: r.wrong_count, createdAt: r.finished_at || r.created_at })),
