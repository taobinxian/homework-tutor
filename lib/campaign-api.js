...
function clearScore(progress) {
  if (!progress) return 0;
  const bestStars = Number(progress.bestStars || 0);
  const clearTimes = Number(progress.clearTimes || 0);
  const result = String(progress.lastResult || '');
  // A completed run must count as at least one clear for unlock purposes even
  // when its computed star score is 0 (for example, a low-accuracy but won
  // combat run). Higher star gates still use bestStars normally.
  const cleared = clearTimes > 0 || result === 'win' || result === 'complete' || bestStars > 0;
  return Math.max(bestStars, cleared ? 1 : 0);
}
...
function rowToLevel(row, progressMap = {}) {
  ...
  const unlocked = required.length === 0 || required.every(id => clearScore(progressMap[id]) >= requiredStars);
  const cleared = clearScore(p) > 0;
  return {
    id: row.id,
...
function mapHandler(db, query = {}) {
  ...
  const eventsResult = fullProduct.listMapEventsHandler(db, { user, status: 'available' });
  const events = eventsResult.body || [];
  const next = levels.find(l => l.unlocked && l.state !== 'cleared') || levels.find(l => l.unlocked) || levels[0] || null;
  fullProduct.trackEvent(db, user, 'campaign_open', { grade, subject, semester, levelCount: levels.length });
...
module.exports = { safeJSON, clearScore, rowToLevel, progressFor, listLevels, mapHandler, detailHandler };
