...
function buildSummary(rows, mastery) {
  const totals = rows.reduce((acc, r) => {
    acc.levels += 1;
    acc.correct += r.correct_count || 0;
    acc.wrong += r.wrong_count || 0;
    acc.durationSec += r.duration_sec || 0;
    if (r.topic) acc.topics[r.topic] = (acc.topics[r.topic] || 0) + (r.wrong_count || 0);
    const day = String(r.day || r.created_at || '').slice(0, 10);
    if (day) {
      acc.days[day] ||= { date: day, levelsCompleted: 0, questionCount: 0, correctCount: 0, wrongCount: 0, learningMinutes: 0 };
      ...
      acc.days[day].learningMinutes += Math.round((r.duration_sec || 0) / 60);
    }
    return acc;
  }, { levels: 0, correct: 0, wrong: 0, durationSec: 0, topics: {}, days: {} });
  const totalQ = totals.correct + totals.wrong;
  const weakTopics = Object.entries(totals.topics)
    .filter(([, n]) => n > 0)
    .sort((a, b) => b[1] - a[1])
    .map(([topic, wrongCount]) => ({ topic, wrongCount, reviewAvailable: true }));
  return {
...
function dailyReportHandler(db, query = {}) {
  ...
  const rows = db.prepare(`SELECT lr.*, cl.topic, cl.title, date(COALESCE(lr.finished_at, lr.created_at)) AS day
                           FROM level_runs lr
                           LEFT JOIN campaign_levels cl ON cl.id=lr.level_id
                           WHERE lr.user=? AND date(COALESCE(lr.finished_at, lr.created_at))=date(?) AND lr.result <> 'started'
                           ORDER BY COALESCE(lr.finished_at, lr.created_at) DESC`).all(user, date);
  const mastery = fetchMastery(db, user, 5);
  const summary = buildSummary(rows, mastery);
...
function weeklyReportHandler(db, query = {}) {
  ...
  const rows = db.prepare(`SELECT lr.*, cl.topic, cl.title, date(COALESCE(lr.finished_at, lr.created_at)) AS day
                           ...
                           LEFT JOIN campaign_levels cl ON cl.id=lr.level_id
                           WHERE lr.user=?
                             AND date(COALESCE(lr.finished_at, lr.created_at)) BETWEEN date(?, '-6 day') AND date(?)
  ...
  return { status: 200, body: {
    ...
    summary: buildSummary(rows, mastery),
    runs: rows.map(r => ({ levelId: r.level_id, title: r.title, topic: r.topic, result: r.result, stars: r.stars, correct: r.correct_count, wrong: r.wrong_count, createdAt: r.finished_at || r.created_at })),
  }};
}
