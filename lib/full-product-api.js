...
function generateBounties(db, { user = 'default', source = 'system' } = {}) {
  ...
  const ins = db.prepare(`INSERT OR IGNORE INTO bounty_tasks
    (id,user,source,topic,monster_id,task_type,difficulty,target_json,progress_json,reward_json,status,created_at)
    VALUES (@id,@user,@source,@topic,@monsterId,@taskType,@difficulty,@target,@progress,@reward,'active',CURRENT_TIMESTAMP)`);
  for (const m of monsters) {
    ...
    const payload = { id: id('bounty', `${user}|${m.topic}|${taskType}|${Date.now()}`), user, source, topic: m.topic, monsterId: m.id, taskType, difficulty, target: JSON.stringify(target), progress: JSON.stringify({ correct: 0, total: 0, accuracy: 0 }), reward: JSON.stringify(bountyReward(difficulty)) };
    const info = ins.run(payload);
    if (info.changes) {
      db.prepare(`UPDATE wrong_monsters SET status='bounty', updated_at=CURRENT_TIMESTAMP WHERE id=? AND status!='purified'`).run(m.id);
      trackEvent(db, user, 'bounty_generated', { topic: m.topic, difficulty, monsterId: m.id });
      created.push(db.prepare('SELECT * FROM bounty_tasks WHERE id=?').get(payload.id));
    } else {
      created.push(db.prepare(`SELECT * FROM bounty_tasks
