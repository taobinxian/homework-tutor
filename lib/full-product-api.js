...
function generateBounties(db, { user = 'default', source = 'system' } = {}) {
  ...
  for (const m of monsters) {
    ...
    const payload = { id: stableId('bounty', `${user}|${m.topic}|${taskType}|active`), user, source, topic: m.topic, monsterId: m.id, taskType, difficulty, target: JSON.stringify(target), progress: JSON.stringify({ correct: 0, total: 0, accuracy: 0 }), reward: JSON.stringify(bountyReward(difficulty)) };
