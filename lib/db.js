...
function ensureBountyTasksSchema(db) {
  const table = db.prepare(`SELECT sql FROM sqlite_master WHERE type='table' AND name='bounty_tasks'`).get();
  if (!table) return;
  const cols = db.prepare(`PRAGMA table_info(bounty_tasks)`).all().map(c => c.name);
  const sql = String(table.sql || '');
  const hasCycle = cols.includes('cycle');
  const hasLegacyStatusUnique = /UNIQUE\s*\(\s*user\s*,\s*topic\s*,\s*task_type\s*,\s*status\s*\)/i.test(sql);
  const hasCycleUnique = /UNIQUE\s*\(\s*user\s*,\s*topic\s*,\s*task_type\s*,\s*cycle\s*\)/i.test(sql);
  if (hasCycle && hasCycleUnique && !hasLegacyStatusUnique) {
    db.exec(`CREATE INDEX IF NOT EXISTS idx_bounty_user_status ON bounty_tasks(user, status, created_at);`);
    return;
  }

  const migrate = db.transaction(() => {
    db.exec(`
      CREATE TABLE bounty_tasks_new (
        id TEXT PRIMARY KEY,
        ...
        claimed_at DATETIME,
        UNIQUE(user, topic, task_type, cycle)
      );
    `);
    const rows = db.prepare(`SELECT * FROM bounty_tasks ORDER BY user, topic, task_type, created_at, id`).all();
    const nextCycleByKey = new Map();
    const usedCyclesByKey = new Map();
    const insert = db.prepare(`INSERT INTO bounty_tasks_new
      (id,user,source,topic,monster_id,task_type,cycle,difficulty,target_json,progress_json,reward_json,status,created_at,completed_at,claimed_at)
      VALUES (@id,@user,@source,@topic,@monster_id,@task_type,@cycle,@difficulty,@target_json,@progress_json,@reward_json,@status,@created_at,@completed_at,@claimed_at)`);
    for (const row of rows) {
      const key = `${row.user}\u0000${row.topic}\u0000${row.task_type}`;
      if (!usedCyclesByKey.has(key)) usedCyclesByKey.set(key, new Set());
      const used = usedCyclesByKey.get(key);
      let cycle = hasCycle ? Number(row.cycle || 0) : 0;
      if (!Number.isInteger(cycle) || cycle < 1 || used.has(cycle)) {
        cycle = nextCycleByKey.get(key) || 1;
        while (used.has(cycle)) cycle += 1;
      }
      used.add(cycle);
      nextCycleByKey.set(key, cycle + 1);
      insert.run({
        id: row.id,
    ...
    db.exec(`
      DROP TABLE bounty_tasks;
      ALTER TABLE bounty_tasks_new RENAME TO bounty_tasks;
...
function initSchema(db) {
  ...
  db.exec(`
    ...
    CREATE INDEX IF NOT EXISTS idx_campaign_sessions_user_status ON campaign_sessions(user, status, updated_at);

    CREATE TABLE IF NOT EXISTS campaign_progress_saves (
    ...
    CREATE INDEX IF NOT EXISTS idx_campaign_progress_user_status ON campaign_progress_saves(user, status, server_updated_at);
    CREATE INDEX IF NOT EXISTS idx_campaign_progress_run ON campaign_progress_saves(run_id);
