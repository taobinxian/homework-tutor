  ...
  const tx = db.transaction(() => {
    for (const a of answers) {
      ...
      if (!a.isCorrect && a.questionText) {
        ...
        fullProduct.trackEvent(db, run.user, 'wrong_question_record', { runId, topic: a.topic, questionId: a.questionId });
      }
      if (a.isCorrect && combo >= 3) fullProduct.trackEvent(db, run.user, 'combo_reach', { runId, combo, topic: a.topic });
    }
    if (newAnswers.length) {
      updateMastery(db, { user: run.user, grade: level.grade, subject: level.subject, semester: level.semester, answers: newAnswers });
      fullProduct.syncMonstersFromWrongbook(db, { user: run.user });
      fullProduct.generateBounties(db, { user: run.user, source: 'settlement' });
    }
  });
  tx();
...
function finishHandler(db, payload = {}) {
  ...
  fullProduct.generateBounties(db, { user, source: 'settlement' });
  const bountySettlements = fullProduct.settleBountiesForRun(db, { user, runId });
  const highlights = fullProduct.recordRunHighlights(db, { user, runId, levelId, stats: payload.combatStats || payload.stats || {}, result, rewards });
  fullProduct.trackEvent(db, user, 'level_finish', { runId, levelId, result, stars, accuracy });
  return { status: 200, body: { ok: true, runId, result, stars, rewards, accuracy, generatedReview: wrong > 0, bountySettlements, highlights } };
}
